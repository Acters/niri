...
Gate 1 passed: render-node-only SCANOUT|RENDERING LINEAR allocation/copy/readback tests for ABGR8888 and ABGR2101010, 24 frames each at 1920x1080/1952x1104 with partial damage and midtones. The example's `--scanout-candidate` requires the modifiers2 feature so GBM receives the requested usage flags and explicit modifier together. These tests cannot establish KMS admissibility or fence acceptance. Atomic TEST_ONLY and actual-display tests need separately coordinated KMS authority; no display takeover has been authorized yet. An opt-in niri diagnostic (`NIRI_VK_KMS_TEST_ONLY_OUTPUT`) is prepared: a delayed background engine creates private candidates, and the compositor uses its existing atomic DRM surface to test them with `test_state(..., false)` only. It refuses legacy/inactive/pending-mode cases, never commits or page-flips a candidate, and tests both a completed copy without a fence and a second copy with a native fence. The diagnostic was explicitly approved and ran once as niri `540230c7`, invocation `604979ed4c7b440b8a1f894bc8a906ef`. Gate 2 passed at 17:15:52 UTC for eDP-1 1920x1080: ABGR8888 exported as opaque XBGR8888 and ABGR2101010 as opaque XBGR2101010, both explicit LINEAR. For each format atomic TEST_ONLY accepted a completed copy without a fence and a second native copy fence with `complete_at_test=false`. No candidate was displayed. The opt-in was removed from the service drop-in after completion so subsequent restarts do not repeat the diagnostic. Log: `/tmp/niri-kms-test-only.log`. On `nvidia-intel-bridge-direct-target`, an opt-in prototype is being implemented after these feasibility gates. The temporary compositor diagnostic is removed from this branch (retained on the scanout-probe branch). No actual direct display has been enabled.

Prototype contract: `NIRI_VK_DIRECT_TARGET=1` enables an otherwise-off policy. Only foreign atomic output swapchains request tested explicit-LINEAR ABGR formats, with normal negotiation fallback. Renderer paired external-framebuffer hooks expose the ORIGINAL allocation and acquire fence, then publish the completed external write into target/shared-texture synchronization without changing pixels. GLES retains original identity through the common DMA-BUF-backed Texture path as well as the Image fallback; ordinary textures remain unsupported. Vulkan copies only exact clipped original damage, never CPU-path bounding-box/full-frame shortcuts. DrmCompositor keeps swapchain/KMS release ownership. Offscreen tests include three original destinations, untouched-pixel sentinels, sparse/L-shaped damage, shared-context reads, capture, GLES writes and blits. Post-submit errors retire copy work before returning a slot.

Prototype validation passed: 16 transfer/lifecycle tests, two probe-model tests, strict Smithay/GLES and niri workspace Clippy, no-Vulkan feature isolation, and 216 niri/config/IPC tests. Actual direct-bound-target traces accompany passing 8/10-bit offscreen probes (54 measured writes each) and a full-HD ten-bit run. They verify original-binding identity, guard drop after cache invalidation, exact outside-damage pixels, shared-context reads, fallback GLES writes, blit-to with resumed source drawing and pre-continuation snapshot, blit-from with retained final fence, resize and invalidation. Independent final review found no remaining proven blocking lifetime/ordering defect. Smithay prototype checkpoint: `0e2ad6d1`. The library policy remains default-off.

Gate 3 passed after a separate explicit approval: niri `9ee3ff77` / Smithay `0e2ad6d1` were installed as `/home/acters/.local/bin/niri-direct-target` and selected by `98-direct-target-test.conf` with `NIRI_VK_DIRECT_TARGET=1`. The real session (invocation `c6ce6200a9eb45129f1a609c5553bcf7`) started at 18:45:48 UTC. eDP-1 negotiated ABGR2101010 with `[Linear]`; logs show actual `submitted direct Vulkan framebuffer transfer` events with AB30, LINEAR and native fences. No render/queue errors were observed; all three outputs remained active (internal ~144 Hz, DP ~240 Hz, HDMI ~75 Hz). The user tested the result and explicitly confirmed it looks correct and should remain enabled.

All requested phases are implemented and locally validated. No per-output renderer migration or dynamic GPU selection was introduced. Broader device-loss/fault-injection, long-term hotplug/soak, and performance/power measurements remain future validation, not claimed results.

Current rollback: remove only `/home/acters/.config/systemd/user/niri.service.d/98-direct-target-test.conf`, then run `systemctl --user daemon-reload` and restart at a coordinated time. This restores `96-initialization-test.conf` and the validated phase-two `niri-lazy-transfer` binary. The temporary KMS diagnostic override was removed. All code changes are on local experimental branches; none have been pushed.
