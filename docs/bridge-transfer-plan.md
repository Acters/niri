...
Prototype validation passed: 16 transfer/lifecycle tests, two probe-model tests, strict Smithay/GLES and niri workspace Clippy, no-Vulkan feature isolation, and 216 niri/config/IPC tests. Actual direct-bound-target traces accompany passing 8/10-bit offscreen probes (54 measured writes each) and a full-HD ten-bit run. They verify original-binding identity, guard drop after cache invalidation, exact outside-damage pixels, shared-context reads, fallback GLES writes, blit-to with resumed source drawing and pre-continuation snapshot, blit-from with retained final fence, resize and invalidation. Independent final review found no remaining proven blocking lifetime/ordering defect. Smithay prototype checkpoint: `0e2ad6d1`. The prototype remains default-off and has not yet been used for actual KMS display.
